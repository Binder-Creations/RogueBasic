...
class AbilityServices {
  ...
  static monsterAttack(ability, pc, monster, monsters, combatUpdates, c){
    for(let i = 0; i < ability.hits; i++){
      if(ability.selfFlags || ability.targetFlags){
      ...
      if(this.pcCorpseCollector(pc, monster, ability, damage, crit, c)){
        break;
      }
    }
  }
  ...
  static pcCorpseCollector(pc, monster, ability, damage, crit, c){
    if(pc.currentHealth <= 0){
      c.gameOver(new CombatUpdate("gameOver", ability.name, "pc", monster.name, damage, crit));
    }
  }
