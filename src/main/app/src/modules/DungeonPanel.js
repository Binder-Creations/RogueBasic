...
class DungeonPanel extends React.Component {
  ...
  render(){
    ...
    let Prefix = this.props.dungeonBoard[this.props.number-1].prefixMod
      ? () => {return(this.setPrefix())}
      : () => {return(<></>)}
    let Postfix = this.props.dungeonBoard[this.props.number-1].postfixMod
      ? () => {return(this.setPostfix())}
      : () => {return(<></>)}

    switch(this.props.dungeonBoard[this.props.number-1].theme){
      case "Arcane":
    ...
    this.boss = this.props.dungeonBoard[this.props.number-1].boss ? this.bossTrue : this.bossFalse;
    this.miniboss = this.props.dungeonBoard[this.props.number-1].miniboss ? this.minibossTrue : this.minibossFalse;
    ...
    return(
      <div className={"dungeon-panel " + saturate + " d" + this.props.number} onClick={this.onClick}>
        ...
        <p className="dungeon-panel-name" style={{fontSize: fontSize}}>{this.props.dungeonBoard[this.props.number-1].name}</p>
        <p className="dungeon-panel-theme">{this.props.dungeonBoard[this.props.number-1].theme}</p>
        <p className="dungeon-panel-cr" title={"This dungeon is expected to be an average challenge for adventurers of level "+this.props.dungeonBoard[this.props.number-1].challengeRating}>{"CR: "+this.props.dungeonBoard[this.props.number-1].challengeRating}</p>
        <p className="dungeon-panel-floorcount" title={"This dungeon has " + this.props.dungeonBoard[this.props.number-1].floorCount + " floor" + s}>{this.props.dungeonBoard[this.props.number-1].floorCount}</p>
        <img className="dungeon-panel-floors" title={"This dungeon has " + this.props.dungeonBoard[this.props.number-1].floorCount + " floor" + s} src={this.props.props.images.dungeonFloors} alt="Floors"/>
        <this.Miniboss/>
        <this.Boss/>
        <Prefix/>
        <Postfix/>
        <p className="dungeon-panel-quest">Quest: Defeat All Enemies</p>
        <p className="dungeon-panel-reward">{"Reward: " + this.props.dungeonBoard[this.props.number-1].reward}</p>
      </div>
    );  
  }

  setPrefix(){
    let type = this.props.dungeonBoard[this.props.number-1].prefixMod.match(/([a-z]+)/)[1]
    let modifier = this.props.dungeonBoard[this.props.number-1].prefixMod.match(/(\d+)/)[1]
    return(
  ...
  setPostfix(){
    let type = this.props.dungeonBoard[this.props.number-1].postfixMod.match(/([a-z]+)/)[1]
    let modifier = this.props.dungeonBoard[this.props.number-1].postfixMod.match(/(\d+)/)[1]
    return(
